...
export const generatePDFReport = (results) => {
    ...
    const addMetricBox = (label, value, x, y, width, color = colors.primary) => {
        // Box background
    ...
    const groupedRecs = recommendationsList.reduce((groups, rec) => {
    ...
    const summaryText = `This analysis of ${results.totalTransactions} transactions reveals ${results.annualSavings > 0 ? 'significant optimization opportunities' : 'your current setup is well-optimized'}. ` +
        `Your effective rate of ${results.effectiveRate?.toFixed(2)}% ${results.effectiveRate > 3 ? 'exceeds' : 'aligns with'} industry benchmarks. ` +
        `${results.failureRate > 3 ? `High failure rate of ${results.failureRate.toFixed(1)}% requires immediate attention. ` : ''}` +
